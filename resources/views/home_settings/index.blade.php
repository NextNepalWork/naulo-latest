...
@section('content')
    ...
    <div class="tab-base">
        ...
        <ul class="nav nav-tabs">
          ...
          <li class="">
                <a data-toggle="tab" href="#demo-lft-tab-6" aria-expanded="false">{{ __('Login Content') }}</a>
            </li>
        </ul>
        ...
        <div class="tab-content">
            ...
            <div id="demo-lft-tab-6" class="tab-pane fade">
                ...
                <div class="row">
                    <div class="col-sm-12">
                        <a onclick="login_content()" class="btn btn-rounded btn-info pull-right">{{__('Add Login Content')}}</a>
                    </div>
                </div>
                ...
                <div class="panel">
                    <div class="panel-heading">
                        <h3 class="panel-title">{{__('Login Content')}} (Max 3 published)</h3>
                    </div>
                    <div class="panel-body">
                        <table class="table table-striped table-bordered demo-dt-basic" cellspacing="0" width="100%">
                            <thead>
                            ...
                            <tbody>
                                @foreach(\App\LoginContent::get() as $key => $banner)
                                    <tr>
                                        <td>{{$key+1}}</td>
                                        <td><img loading="lazy"  class="img-md" src="{{ asset($banner->photo)}}" alt="banner Image"></td>
                                        <td>{{ __('Banner Position ') }}{{ $banner->position }}</td>
                                       <td>{{ $banner->head }}</td>
                                      <td>{{ $banner->subheader }}</td>
                                      <td>{{ $banner->body }}</td>
                                        <td><label class="switch">
                                            <input onchange="update_login_content(this)" value="{{ $banner->id }}" type="checkbox" <?php if($banner->published == 1) echo "checked";?> >
                                            <span class="slider round"></span></label></td>
                                        ...
                                        <td>
                                            <div class="btn-group dropdown">
                                                ...
                                                <ul class="dropdown-menu dropdown-menu-right">
                                                    <li><a onclick="edit_login_content({{ $banner->id }})">{{__('Edit')}}</a></li>
                                                    <li><a onclick="confirm_modal('{{route('login_content.destroy', $banner->id)}}');">{{__('Delete')}}</a></li>
                                                </ul>
                                            </div>
                                        </td>
                                    </tr>
...
<script type="text/javascript">
    ...
    function login_content(){
        ...
        $.get('{{ route('login_content.create', 1)}}', {}, function(data){
   ...
   function edit_login_content(id){
        var url = '{{ route("login_content.edit", "home_banner_id") }}';
        url = url.replace('home_banner_id', id);
        $.get(url, {}, function(data){
            $('#demo-lft-tab-6').html(data);
            $('.demo-select2-placeholder').select2();
    ...
    function update_login_content(el){
        if(el.checked){
            var status = 1;
        }
        else{
        ...
        $.post('{{ route('login_content.update_status') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
            if(data == 1){
                showAlert('success', 'Content status updated successfully');
            }
            else{
                showAlert('danger', 'Maximum 4 banners to be published');
            }
